...
export default function Requests() {
  const [form, setForm] = useState({
    name: '', phone: '', building: '', apartment: '', content: ''
  })
  ...
  const handleSubmit = async () => {
    ...
    setStatus('success')
    setForm({ name: '', phone: '', building: '', apartment: '', content: '' })
  }
  ...
  return (
    <div className="card">
      ...
      <div style={{marginBottom:'14px'}}>
        <div style={{fontSize:'12px', fontWeight:'700', color:'var(--muted)', marginBottom:'6px'}}>שם מלא *</div>
        <input
          value={form.name}
      ...
      {/* Phone */}
      <div style={{marginBottom:'14px'}}>
        <div style={{fontSize:'12px', fontWeight:'700', color:'var(--muted)', marginBottom:'6px'}}>טלפון *</div>
        <input
          value={form.phone}
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'14px'}}>
        <div>
          <div style={{fontSize:'12px', fontWeight:'700', color:'var(--muted)', marginBottom:'6px'}}>בניין *</div>
          <select value={form.building} onChange={e => update('building', e.target.value)} style={inputStyle}>
        ...
        <div>
          <div style={{fontSize:'12px', fontWeight:'700', color:'var(--muted)', marginBottom:'6px'}}>מספר דירה *</div>
          <input
      ...
      <div style={{marginBottom:'20px'}}>
        <div style={{fontSize:'12px', fontWeight:'700', color:'var(--muted)', marginBottom:'6px'}}>תוכן הפנייה *</div>
        <textarea
          value={form.content}
          onChange={e => update('content', e.target.value)}
          placeholder="תארו את הבעיה או הבקשה..."
          rows={5}
      ...
      {/* Status messages */}
      {status === 'missing' && (
        <div className="info-block amber" style={{marginBottom:'14px'}}>⚠️ נא למלא את כל השדות.</div>
      )}
      {status === 'error' && (
